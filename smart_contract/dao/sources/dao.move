module dao::dao {
    ...
    struct DAO has drop{}
    struct AgentDao<phantom T> has key{
        id: UID,
    ...
    fun init(witness: DAO, ctx: &mut TxContext) {
        ...
        let (treasury_cap,metadata) = coin::create_currency<DAO>(witness,18,b"DAO",b"dao",b"AgentDao token.",option::none(),ctx);
    ...
    //===========Access Contral==========
    public fun distribute_membercap(core_cap:& CoreCap, receiver:address, dao:&mut AgentDao<DAO>, ctx:&mut TxContext){
        check_corecap_role(core_cap,ctx);
    ...
    public fun total_members(dao: &AgentDao<DAO>):u64{
        dao.total_members
    }
    ...
    public fun test() {
        ...
        //5. distribute membercap
        {
            test_scenario::next_tx(&mut scenario, addr1);
            let core_cap = test_scenario::take_from_sender<CoreCap>(& scenario);
            let dao = test_scenario::take_shared<AgentDao<DAO>>(& scenario);
